...
public class ReportManagement {
    ...
    public static String downloadURL(String urlInStr, String folderOut, MongoClientURI mongoURI) throws IOException {
        ...
        if (report != null) {
            ...
            try {
                ...
                // Check if image has an alpha channel (transparent PNG)
                if (downloadedImage.getColorModel().hasAlpha()) {
                    System.out.println("Downloaded image has an alpha channel, converting to RGB format");
                    
                    // Create a new RGB image with white background
                    BufferedImage rgbImage = new BufferedImage(
                        downloadedImage.getWidth(),
                        downloadedImage.getHeight(),
                    ...
                    g2d.fillRect(0, 0, downloadedImage.getWidth(), downloadedImage.getHeight());
                    
                    // Draw the original image over the white background
                    g2d.drawImage(downloadedImage, 0, 0, null);
                    g2d.dispose();
                    
                    // Save the RGB version back to the source file
                    ImageIO.write(rgbImage, "png", imageFile);
                    
                    // Update the downloaded image reference
                    downloadedImage = rgbImage;
                    System.out.println("Successfully converted transparent PNG to RGB format");
                }
    ...
    public static String reportCalculation(String urlHash, MongoClientURI mongoURI , String folderOut, int maxGhostImageSmallDimension, int numGhostThreads, int numberOfThreads, long computationTimeoutLimit) throws UnknownHostException{
        ...
        ManipulatedScoreReport manipulatedScoreReport=new ManipulatedScoreReport();
        ...
        File manipulatedScoreOutputFile = new File(baseFolder, "ManipulatedScoreOutput.png");
        try {
            BufferedImage sourceImage = ImageIO.read(new File(report.sourceImage));
            
            // If image has an alpha channel (transparent PNG), convert it to RGB
            if (sourceImage.getColorModel().hasAlpha()) {
                System.out.println("Image has an alpha channel, converting to RGB format");
                ...
                // Save the RGB version back to the source file
                ImageIO.write(rgbImage, "png", new File(report.sourceImage));
                
                // Update the source image reference
            ...
            // Process the image normally
                ...
                Boolean dqSaved =false, noiseDWSaved=false, ghostSaved=false, elaSaved =false, blkSaved =false, medianNoiseSaved =false, gridsSaved = false, manipulatedScoreSaved = false;
                ...
                // Add manipulated score thread
                ManipulatedScoreThread manipulatedScoreThread = new ManipulatedScoreThread(report.sourceImage, manipulatedScoreOutputFile);
                Future manipulatedScoreFuture = threadpool.submit(manipulatedScoreThread);
                ...
                while (!dqFuture.isDone() | !noiseDWFuture.isDone() |
                    ...
                    if (manipulatedScoreFuture.isDone() & !manipulatedScoreSaved){
                        report.manipulatedScoreReport =(ManipulatedScoreReport) manipulatedScoreFuture.get();
                        manipulatedScoreSaved =true;
                        ds.save(report);
                        System.out.println("Manipulated Score Done");
                    }
                    if ((System.currentTimeMillis()-startTime) > computationTimeoutLimit){
                        ...
                        manipulatedScoreFuture.cancel(true);
                        break;
                    }
                }
                threadpool.shutdown();
